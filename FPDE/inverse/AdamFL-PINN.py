...
# np.savez("./data/AdaFL_PINN_solution.npz", u_pred=u_pred, u_true=u_true_np, x=x_values, t=t_values, alpha=model.alpha.item())

# Move model output to CPU before converting to numpy
u_pred_plot = model(X_input).detach().cpu().numpy().reshape(N, M)
...
fig.savefig('./figures/AdamFL-PINN-solution.png', dpi=200, bbox_inches='tight')
print("Saved figure to ./figures/AdamFL-PINN-solution.png")
plt.show()
...
ax_error.set_title('AdamFL-PINN Absolute Error')
fig_error.colorbar(error_surface, ax=ax_error, shrink=0.6, aspect=12, pad=0.1)

plt.tight_layout()
fig_error.savefig('./figures/AdamFL-PINN-absolute-error.png', dpi=200, bbox_inches='tight')
print("Saved absolute error figure to ./figures/AdamFL-PINN-absolute-error.png")
plt.show()
...
scipy.io.savemat('./data/AdamFL-PINN-solution.mat', {
    'u_pred': model(X_input).detach().cpu().numpy().reshape(N, M),
...
print("Saved solution to ./data/AdamFL-PINN-solution.mat")
