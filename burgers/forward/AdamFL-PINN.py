...
def df(x):
    ...
    loss.backward()
    return np.concatenate([p.grad.cpu().numpy().flatten() if p.grad is not None else np.zeros(p.numel()) for p in pinn_model.net.parameters()])
...
def dh(x):
    ...
    loss.backward()
    grad = np.concatenate([p.grad.cpu().numpy().flatten() if p.grad is not None else np.zeros(p.numel()) for p in pinn_model.net.parameters()])
    return grad.reshape(1, -1)




# Override AdamFL_PINN: physics objective, boundary constraint
def AdamFL_PINN(f, h, df, dh, x_start, Kp, Ki=0, eta=0.01, max_iter=1000, tol=1e-6):
    ...
    pbar = trange(max_iter, desc="AdamFL-PINN Training")
    ...
    return x, history, lambda_boundary_history, loss_epoch_history, loss_boundary_history, loss_physics_history, loss_total_history
...
x_final, history, lambda_boundary_history, loss_epoch_history, loss_boundary_history, loss_physics_history, loss_total_history = AdamFL_PINN(
    f, h, df, dh,
...
np.savez(
    "./training_data/AdamFL_PINN_physics_loss_history.npz",
...
print("Saved loss history to ./training_data/AdamFL_PINN_physics_loss_history.npz")
...
final_boundary = h(x_final)
final_physics = f(x_final)

print("\n--- Optimization Finished ---")
# print(f"Final Boundary Loss (Constraint 1): {final_boundary[0]:.4f}")
# print(f"Final Physics Loss (Objective): {final_physics:.4f}")
save_model(pinn_model.net, "./trained_AdamFL_PINN_model.pth")
...
load_model(pinn_model.net, "./trained_AdamFL_PINN_model.pth")
...
plt.xlabel(r"$t$"); plt.ylabel(r"$x$"); plt.title("AdamFL-PINN Solution")
...
output_filename = "./figures/solution/AdamFL_PINN_solution.png"
...
try:
    scipy.io.savemat('./data/AdamFL_PINN_solution.mat', {'solution': Unp_pred, 'x': xnp_plot, 't': tnp_plot})
    print('Saved solution matrix to ./data/AdamFL_PINN_solution.mat')
...
output_filename_err = "./figures/solution/AdamFL_PINN_error.png"
...
try:
    scipy.io.savemat('./data/AdamFL_PINN_error.mat', {'error': error_grid, 'x': xnp_plot, 't': tnp_plot})
    print('Saved error matrix to ./data/AdamFL_PINN_error.mat')
except Exception as e:
...
plot_optimization_history_multi_constraint(full_history, legends=['AdamFL-PINN'], linewidth=3, savename='./figures/FL-PINN_training_history.png')
