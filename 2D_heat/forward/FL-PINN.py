...
def plot_solution_comparison(model, t_value, save_solution=True, save_error=True):
    ...
    # savemat(f"./mat_files/fl_solution_comparison_t{t_value}.mat", {
    #     "X": X_np, "Y": Y_np, "U_pred": U_pred_np,
    #     "U_exact": U_exact_np, "Error": Error_np
    # })

    # Save separate solution/error files with requested naming.
    ...
    if save_solution:
        ...
        print(f"Saved ./data/FL_solution_t{t_value}.mat")
    if save_error:
        savemat(f"./data/FL_error_t{t_value}.mat", {
            "X": X_np,
            "Y": Y_np,
            "Error": Error_np,
        })
        print(f"Saved ./data/FL_error_t{t_value}.mat")

    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle(f'FL-PINN Comparison at t = {t_value}', fontsize=16)
    ...
    fig.savefig(f"./figures/fl_comparison_t{t_value}.png", dpi=300, bbox_inches='tight')
    plt.show()
