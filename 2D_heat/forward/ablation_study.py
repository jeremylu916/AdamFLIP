...
# --- SOFL Optimizer ---
def AdamFL_PINN(f, h, df, dh, x_start, Kp, Ki=0, eta=0.01, max_iter=1000, tol=1e-6, weight_decay=0.0, verbose=False):
...
# ================================
# ABLATION STUDY: Multiple Kp Values with Multiple Runs
# ================================
...
# Kp values to test (ablation study)
Kp_values = [10.0, 50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0, 5000.0]

# Dictionary to store results for each Kp (indexed by [kp_val][seed_idx])
ablation_results = {kp_val: [] for kp_val in Kp_values}

# Loop over different random seeds THEN different Kp values
for seed_idx in range(NUM_SEEDS):
    ...
    # Use the same initial network parameters for every Kp within this seed.
    ...
    for kp_val in Kp_values:
        print(f"  Kp = {kp_val:.1f}...", end=" ")
        
        # Re-initialize the model for each Kp, then restore the shared start.
        ...
        x_final, history = AdamFL_PINN(
            f, h, df, dh,
            x_current, Kp_matrix, Ki=Ki, eta=eta,
            max_iter=NUM_ITERATIONS, tol=tol
        )
        
        # Print and store final losses
...
shaded_kp_values = [10.0, 500.0, 1000.0]

# Extract and plot BC loss for each Kp with mean ± std across seeds
for idx, kp_val in enumerate(Kp_values):
    histories = [result['history'] for result in ablation_results[kp_val]]
...
plt.savefig('./figures/ablation_study_bc_loss.png', 
            dpi=300, 
            bbox_inches='tight', 
            facecolor='white',
            edgecolor='none')
print("✓ Saved: ./figures/ablation_study_bc_loss.png")
plt.show()
...
# Save aggregated statistics for each Kp across all seeds
for kp_val in Kp_values:
    histories = [result['history'] for result in ablation_results[kp_val]]
    ...
    final_physics_losses = np.array([result['physics_loss'] for result in ablation_results[kp_val]])
    final_ic_losses = np.array([result['ic_loss'] for result in ablation_results[kp_val]])
    final_bc_losses = np.array([result['bc_loss'] for result in ablation_results[kp_val]])
    
    np.savez(
        f"./training_data/ablation_Kp_{kp_val:.1f}_history.npz",
        ...
        final_ic_loss_mean=np.mean(final_ic_losses),
        final_ic_loss_std=np.std(final_ic_losses),
...
print("="*70)
print(f"\nConfiguration:")
print(f"  • Tested Kp values: {Kp_values}")
print(f"  • Random seeds: {NUM_SEEDS}")
print(f"  • Training iterations per run: {NUM_ITERATIONS}")
print(f"  • Output: ./figures/ablation_study_bc_loss.png")
print(f"  • Data: ./training_data/ablation_Kp_*.npz")
print(f"\nKey observations:")
print(f"  • Higher Kp values drive boundary loss down faster")
print(f"  • Diminishing returns visible beyond Kp=500")
...
# Calculate and display final loss statistics for each Kp
print(f"\nFinal Loss Statistics:")
for kp_test in Kp_values:
    final_physics_losses = np.array([result['physics_loss'] for result in ablation_results[kp_test]])
    final_ic_losses = np.array([result['ic_loss'] for result in ablation_results[kp_test]])
    final_bc_losses = np.array([result['bc_loss'] for result in ablation_results[kp_test]])
    print(
        f"  Kp={kp_test:.0f}: "
        f"Physics Loss = {np.mean(final_physics_losses):.4e} ± {np.std(final_physics_losses):.4e}, "
        f"IC Loss = {np.mean(final_ic_losses):.4e} ± {np.std(final_ic_losses):.4e}, "
...
caption = (
    ...
    f" different proportional penalty gains $K_p$. Lines represent mean over"
    f" {NUM_SEEDS} independent random seeds; shaded regions show ±1 standard"
    f" deviation. Higher $K_p$ values accelerate constraint satisfaction,"
    f" with diminishing returns beyond $K_p=500$."
)
print(caption)
print("─"*70 + "\n")
