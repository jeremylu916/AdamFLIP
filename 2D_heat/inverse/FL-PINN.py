...
def FL_PINN(f, h, df, dh, x_start, Kp, Ki=0, eta=0.01, max_iter=1000, tol=1e-6, weight_decay=0.0):
    ...
    loss_file_path = "./training_logs/FL-PINN_inverse_training_loss.txt"
    with open(loss_file_path, "w") as log_file:  # <-- renamed from f → log_file
        ...
        pbar = trange(max_iter, desc="FL-PINN")
...
print("Starting Adam warm start training for 3000 epochs...")
...
adam_pbar = trange(ADAM_EPOCHS, desc="FL Adam warmup")
...
# Save FL loss history (every 100 iters, from `history`)
...
def plot_solution_comparison(model, t_value):
        ...
        savemat(f"./data/FL_solution_t{t_value}.mat", {
            "X": X_np,
            "Y": Y_np,
            "U_pred": U_pred_np,
            "U_exact": U_exact_np,
        ...
        print(f"Successfully saved data to ./data/FL_solution_t{t_value}.mat")
        ...
        fig.savefig(f"./figures/pinn_FL_inverse_comparison_t{t_value}.png", dpi=300, bbox_inches='tight')
        plt.show()
...
plot_optimization_history_5_plots(
    history, 
    legends=["SOFL+Adam"], 
    savename="./figures/SOFL_loss_history.png"
)
